...
import random
from GeometricLocalPath import path_vertical

def tsp_path(initial_point, path_set, path_l_set, path_sf_set):
    n_v = len(path_l_set)
    ...
    T_path_tsp = []
    if path_order[0] == 0:
        T_path_tsp = [[initial_point[0]],[initial_point[1]]]
        for i in range(len(path_order)-1):
...
def ga_tsp_path(a, initial_point, line_set):
    ...
    for i in range(len(line_set)):
        [new_path, l, sf ] = path_vertical(line_set[i], flag = a[i]+1)
        path_set.append(new_path)
        path_l_set.append(l)
        path_sf_set.append(sf)

    (TL_tsp, _, _) = tsp_path(initial_point, path_set, path_l_set, path_sf_set)
    return -TL_tsp/1000